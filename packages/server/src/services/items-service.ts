...
import { db } from "../db";
import { userItems, type ItemStatus } from "../db/schema";
...
export class ItemsService {
  ...
  static async getUserItems(userId: string, options: GetItemsOptions = {}) {
    try {
      const { status = "active", limit = 50, offset = 0, orderBy = "createdAt", orderDirection = "desc" } = options;
   ...
   */
  static async countUserItems(userId: string, status?: (typeof ItemStatus.enumValues)[number]) {
    try {
      const conditions = [eq(userItems.userId, userId)];
